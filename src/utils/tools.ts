...
// 时间格式
const timeFormat = (time: number | Date | string, format = 'YYYY-MM-DD HH:mm:SS'): string => {
  ...
  //  当前时间与传入时间的时间差
  const timeApart = dateTime - newDateTime
  const timeStampDifference: number =
    moment(moment(date).format('YYYY-MM-DD')).valueOf() - moment(moment(newDate).format('YYYY-MM-DD')).valueOf()
  const ONE_DAY_TIMESTAMP: number = 86400000

  if (timeApart < 60 * 1000) {
    // 小于1分钟
    return '刚刚'
  } else if (timeApart >= 60 * 1000 && timeApart < 60 * 1000 * 60) {
    // 大于一分钟且小于一小时时
    return Math.floor(timeApart / 60 / 1000) + '分钟前'
    ...
    // 日期相隔两天且相差相遇2天时
    return '前天'
  } else if (date.getFullYear() === newDate.getFullYear() && date.getMonth() === newDate.getMonth()) {
    // 当在同一年的同一月
    return date.getDate() - newDate.getDate() + '天前'
  } else if (date.getFullYear() === newDate.getFullYear()) {
    // 当在同一年时
    return date.getMonth() - newDate.getMonth() + '月前'
  } else {
    // 相差一年以上时
    return date.getFullYear() - newDate.getFullYear() + '年前'
  }
}
